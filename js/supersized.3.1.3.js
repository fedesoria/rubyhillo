...
(function($){
	...
	$.supersized = function( options ) {
		...
		$(window).load(function(){
			...
			if(containerStat==0){
			$('#contentContainer').hide();	
			$('#contentContainer').fadeIn('slow');   													//Fade in main container
			$("#barRight").width(0);																	//Set bar width to 0
			$("#barRight").delay(animationDelay).animate({ width: percentComplete }, barSpeed);			// Animate  bar
			$(".counter").html(percentComplete +"&#8204 complete");										// Set counter = to % complete
			$(".counter").delay(1000).fadeIn( 2500);													// Fade in counter
			}else{
